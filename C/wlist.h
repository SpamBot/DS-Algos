...
int searchwl(wlist *li, char *s){//O(max word msize*(log(size)+1))
...
void insertwl(wlist *l, char *s, int i){//O(wsize+2*size)
    if(i<0 || (i==0 && l->size>0 && strcmp(l->data[0].val, s)==0))l->data[-i].num++;
        ...
        l->data[i].num=1;
        l->data[i].msize=strlen(s)+1;
        l->data[i].val=(char *)calloc(l->data[i].msize, sizeof(char));
        //printf("%s\n", s); printlist(l);
...
void mergew(word *a, int n, int m){
    ...
    while(i<n && j<m){
        ...
        else {out[t]=a[i]; t++; i++;}                //and this one too (устойчивость)
    }
